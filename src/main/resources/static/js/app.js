document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btnConfirmLayout) {
        btnConfirmLayout.onclick = () => {
            currentPhotos = [];
            updateLivePreview(); // Show empty layout frame
    ...
    async function handleMultiSnap(total) {
        ...
        if (multiSnap) multiSnap.disabled = true;
        ...
        if (multiSnap) multiSnap.disabled = false;
    ...
    async function renderLayout(targetCanvas, targetCtx) {
        ...
        const totalSlots = (currentLayout === 'horizontal-2') ? 2 : 4;
